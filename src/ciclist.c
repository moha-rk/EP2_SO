...
void *run(void *ciclist)
{
    ...
    {
        while (cont[c->id] == 0)
            usleep(10); //Quantia de sleep apenas para testes
        cont[c->id] = 0;
        if (!c->running)
            break; //Para caso seja eliminado com a thread parada acima
                ...
                if (c->x_pos == 0)
                {
                    ...
                    if (c->laps > 0)
                    {
                        if (quebrou(c))
                        {
                            ...
                            for (int i = c->laps; i <= 2 * ciclists_number; i++)
                                placar[i][0]--;
